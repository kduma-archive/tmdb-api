...
subscriber responds with mocked responses, the ``GuzzleHttp5\Subscriber\History``
...
``GuzzleHttp5\Ring\Client\MockHandler`` as the handler of a client to return the
...
.. warning::
    ...
    when it comes to the node.js test server or the ``GuzzleHttp5\Tests\Server``
...
``GuzzleHttp5\Tests\Server`` provides a static interface to the test server. You
...
response message, a ``GuzzleHttp5\Message\ResponseInterface``, or an array of
HTTP message strings / ``GuzzleHttp5\Message\ResponseInterface`` objects.
...
array of ``GuzzleHttp5\Message\RequestInterface`` objects.
